...
| 差评文本主题 | 差评第一痛点是物流（提及率 34.5%，好评仅 15.6%）；售后/客服提及率为好评的 **7 倍**——出问题后的处理是第二短板 |
| 3C 竞争位 | 手机中位价仅 30 元（全平台 74.9 元）——平台 3C 是低价配件市场；电脑配件差评率 17.00% 为 3C 内最高 |
| 订单全链路 | 下单→签收转化 **97.02%**（99,441 单中 96,476 单签收）；链路 11.5 天里 **8.9 天（77%）在末端配送**；最大流失点在"批准→交承运商"（1,623 单） |
| 用户复购与分层 | 复购率仅 **3.12%**（典型的拉新驱动生意）；但复购客户人均消费是一次性客户的 1.8~3.1 倍，三次及以上客户人均 420.91 元、差评占比最低（10.89%） |
...
**方法**：以商品行为分析单位（一单多品类则分别计入各品类），把订单评分聚合到商品行；九个模块依次回答：品类结构（卖什么）、价格带（卖多少钱）、区域市场（在哪卖）、品类×履约联合（口碑差是配送还是品类自身）、销量预测（趋势外推）、差评文本主题（用户不满什么）、3C 竞争位（平台内定位）、订单全链路漏斗（链路转化与耗时）、用户复购与分层（客户价值）。价格带边界用分布拐点 + 业务整数（0/20/50/100/200）确定，并用第二套边界（0/30/60/120/240）做敏感性检验。
...
- `market/analysis.py` 数据层（底座 + 模块①-⑨：品类/价格带/区域/履约联合/销量预测/差评文本/3C/订单漏斗/用户复购）
- `market/charts.py` 展现层（品类 × 履约联合 2 张 + 整页汇总 1 张）
- `market/品类履约联合_交互看板.html` 单文件交互看板（零依赖、离线可用、双击即开）
- `market/data/` 品类结构、价格带、区域画像、区域品类、品类履约联合、品类销量预测、差评主题、3C品类竞争位、订单漏斗、用户复购分层结果表
- `market/图表/` 13 张图（品类定位/价格带/区域市场/区域品类偏好、履约联合 3 张、预测/文本/3C/漏斗/复购 各 1 张）+ 1 张 Power BI 看板页截图（`看板_品类履约.png`）
...
| 订单全链路漏斗 | 用户复购与分层 |
|:--:|:--:|
| ![订单漏斗](market/图表/订单漏斗图.png) | ![用户复购](market/图表/用户复购分层图.png) |
...
│   └─ 图表/                     6 张分析图 + 4 张看板截图
├─ market/                       方向二：商品市场
│   ├─ analysis.py               数据层（底座 + 模块①-⑦）
│   ├─ charts.py                 展现层（7 张图 + 交互看板 HTML）
│   ├─ 品类履约联合_交互看板.html  单文件交互看板（charts.py 生成）
│   ├─ 报告_商品市场.md
│   ├─ data/                     品类结构、价格带、区域画像、区域品类、履约联合、销量预测、差评主题、3C竞争位、订单漏斗、用户复购分层
│   └─ 图表/                     13 张图（联合分析 3 张 + 预测/文本/3C/漏斗/复购 5 张）+ 1 张 Power BI 看板截图
```
...
#    输出：market/data/ 10 张结果表（品类结构/价格带/区域画像/区域品类/履约联合/销量预测/差评主题/3C竞争位/订单漏斗/用户复购分层）
#         + market/图表/ 13 张图 + 交互看板 HTML + Power BI 导入表（品类履约联合_PowerBI.csv）
...
> 仓库内每个引用数字均可由 `logistics/analysis.py`、`market/analysis.py` + `data/` 结果复现；如对任何数字有疑问，可用代码验证。
