...
**方法**：以商品行为分析单位（一单多品类则分别计入各品类），把订单评分聚合到商品行；七个模块依次回答：品类结构（卖什么）、价格带（卖多少钱）、区域市场（在哪卖）、品类×履约联合（口碑差是配送还是品类自身）、销量预测（趋势外推）、差评文本主题（用户不满什么）、3C 竞争位（平台内定位）。价格带边界用分布拐点 + 业务整数（0/20/50/100/200）确定，并用第二套边界（0/30/60/120/240）做敏感性检验。
...
- `market/analysis.py` 数据层（底座 + 模块①-⑦：品类/价格带/区域/履约联合/销量预测/差评文本/3C）
- `market/charts.py` 展现层（品类 × 履约联合 2 张 + 整页汇总 1 张）
- `market/品类履约联合_交互看板.html` 单文件交互看板（零依赖、离线可用、双击即开）
- `market/data/` 品类结构、价格带、区域画像、区域品类、品类履约联合、品类销量预测、差评主题、3C品类竞争位结果表
- `market/图表/` 11 张图（品类定位/价格带/区域市场/区域品类偏好、履约联合分析 3 张、预测/文本/3C 各 1 张）+ 1 张 Power BI 看板页截图（`看板_品类履约.png`）
...
│   ├─ 报告_商品市场.md
│   ├─ data/                     品类结构、价格带、区域画像、区域品类、履约联合、销量预测、差评主题、3C竞争位
│   └─ 图表/                     11 张图（联合分析 3 张 + 预测/文本/3C 3 张）+ 1 张 Power BI 看板截图
```
...
#    输出：market/data/ 8 张结果表（品类结构/价格带/区域画像/区域品类/履约联合/销量预测/差评主题/3C竞争位）
#         + market/图表/ 11 张图 + 交互看板 HTML + Power BI 导入表（品类履约联合_PowerBI.csv）
